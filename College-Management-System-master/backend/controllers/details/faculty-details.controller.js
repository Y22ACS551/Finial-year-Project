...
const loginFacultyController = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ _id: user._id,role: "Faculty"}, process.env.JWT_SECRET, {
      expiresIn: "1h",
    });
...
const sendFacultyResetPasswordEmail = async (req, res) => {
  try {
    ...
    /*await sendResetMail(user.email, resetId._id,/*facultyDetails.email, "Faculty Password Reset",
      ` <h3>Password Reset</h3><p>Click the link below to reset your password:</p><a href="${resetUrl}">${resetUrl}</a>`
    );*/
